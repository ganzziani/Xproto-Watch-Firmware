...
//  Next:          Current:
    ...
    Mdefault,      // MMAIN1 Menu Select 1 - Channel
...
/*            if(testbit(Misc,autosend)) {    // end of frame
        ...
        if(!testbit(Mcursors,roll)) {
            ...
            if(testbit(MFFT, scopemode) && !testbit(Display, persistent)) {
                if((Index&0x01)==0) {      // erase next vertical line when the sample index is even
					u8CursorY=0;
...
//  DMA.CH1.SRCADDR2  = 0;
    ...
    uint16_t i=0;
    while(!testbit(DMA.CH0.CTRLB,4)) {   // Check transfer complete flag (Capture one full buffer of pre-trigger samples)
        _delay_us(3);
        i++;
        if(testbit(MStatus,update)) {
